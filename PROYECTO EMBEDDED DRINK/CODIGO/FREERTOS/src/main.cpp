...
long distancias[4];
volatile int alturas[4] = {1,1,1,1};
...
// Función para actualizar las distancias y enviar al servidor (aplicacion) al inicio
...
// Función para actualizar las distancias en el servidor
void Height(int index) {
        if(distancias[index] < 14){
            alturas[index] = 140 - distancias[index] * 10;
        } else {
            alturas[index] = 1;
        }
}
            ...
            while (true) {
                ...
                if (tecla||tecla_digital) {
                    if (tecla >= 'A' && tecla <= 'D') {
                        ...
                        distancias[index_digital] = readUltrasonicDistance(trig_pins[index_digital], echo_pins[index_digital]);
                        Height(index_digital);
                        tecla_digital = ' ';
                        Serial.println("Se lleno vaso");
                    } 
                    else if (tecla == 'T') {
